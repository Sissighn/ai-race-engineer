...
def load_single_session_results(year: int, event_key: str, session_type: str) -> Optional[pd.DataFrame]:
    """
    ...
    try:
        ...
        if not keep_cols:
            logger.warning(f"No expected columns found in results for {event_key} {session_type}")
            return None
        ...
        logger.info(f"Successfully loaded {len(df)} results for {session_type} at {event_key}")
